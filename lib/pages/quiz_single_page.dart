...
class _QuizSinglePageState extends State<_QuizSinglePage> {
  ...
  void _initDB() async{
    try {
      ...
      await db.execute('''CREATE TABLE IF NOT EXISTS quizResult (
  ...
  void answer(String key){
    setState(() {
      ...
      if(qIndex<questionList.length-1){
        ...
        if( _quizTimer != null )
          _quizTimer.cancel();
        if( _quizTimerDuration != null )
          _quizTimerDuration.cancel();
        Future insertedId = _insertQuizResult('quizResult', {"weapon":widget.weapon, "type":type, "qa":jsonEncode(answerList), "numQa":answerList.length, "numC":score, "duration":_quizTimerDurationCount, "datetime":dateFormat(DateTime.now())});
        insertedId.then((value)=>{
          print(value)
        });
        Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (BuildContext context)=>QuizResultPage(data: answerList, weapon: widget.weapon, type: keyNum, duration: _quizTimerDurationCount,)));
      }
    });
    prepareQA();
  }
  Timer _quizTimer, _quizTimerDuration;
  int _quizTimerCount, _quizTimerDurationCount = 0;
  void _quizTimerFunc(){
    _quizTimer = Timer(new Duration(seconds: 1), () {
      if(_quizTimerCount > 0){
        setState(() {
          _quizTimerCount--;
        });
        _quizTimerFunc();
  ...
  void _quizTimerDurationFunc(){
    _quizTimerDuration = Timer(new Duration(seconds: 1), () {
      setState(() {
        _quizTimerDurationCount++;
      });
      _quizTimerDurationFunc();
    });
  }
  @override
  void dispose() {
                     ...
                     Container(
                      ...
                      child: ClipRRect(
                        ...
                        child: Container(
                          color: Colors.pinkAccent[100],
                          child: Center(
                            child: new Text("$_quizTimerCount", style: TextStyle(color: Colors.white),),
                          ),
                        ),
                      ),
                    ),
